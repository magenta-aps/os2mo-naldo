/**
 * Tests for normalizeForm utilities.
 *
 * These functions flatten GraphQL response objects into simple key-value objects
 * used to pre-fill edit/terminate forms. Each normalizer extracts specific fields
 * (uuid, name, validity dates, etc.) from the nested GraphQL response shape.
 *
 * The expected outputs define the contract that forms rely on. If the GraphQL
 * response shape changes (e.g. migrating to _response objects), these tests
 * ensure the normalized output stays the same.
 */
...
describe("normalizeEngagement", () => {
  it("flattens org_unit uuid, job_function/engagement_type names, and strips time from 'to' date", () => {
  ...
  it("defaults to null/empty when fields are missing (e.g. empty org_unit array, null references)", () => {
    const result = normalizeEngagement({
      validity: { to: null },
      org_unit: [],
      job_function: null,
...
describe("normalizeAssociation", () => {
  it("extracts person/org_unit uuids and resolves names for type, substitute, trade_union, primary", () => {
...
describe("normalizeITUser", () => {
  it("extracts itsystem name, user_key, primary, external_id, and passes through note argument", () => {
...
describe("normalizeAddress", () => {
  it("maps address_type name, value (from 'name' field), user_key, and visibility name", () => {
...
describe("normalizeManager", () => {
  it("extracts person/org_unit uuids, type/level names, and maps responsibilities array to names", () => {
...
describe("normalizeOwner", () => {
  it("extracts owner uuid as 'person'", () => {
    expect(
      normalizeOwner({
        validity: { to: "2025-01-01T00:00:00" },
        owner: [{ uuid: "p-1" }],
  ...
  it("returns undefined person when owner array is empty (vacant ownership)", () => {
...
describe("normalizeLeave", () => {
  it("extracts leave_type name and engagement uuid", () => {
    expect(
      normalizeLeave({
...
describe("normalizeKLE", () => {
  it("formats kle_number as 'user_key - name' and maps kle_aspects to name array", () => {
    expect(
      normalizeKLE({
        validity: { to: null },
...
describe("normalizeOrganisation", () => {
  it("extracts parent uuid, unit_type/level/time_planning names, and preserves name/user_key", () => {
    expect(
      normalizeOrganisation({
        validity: { to: null },
        name: "IT-afdelingen",
...
describe("normalizeRolebinding", () => {
  it("extracts role name from first element in role array", () => {
    expect(
      normalizeRolebinding({
