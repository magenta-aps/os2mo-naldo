...
{:else}
  {#each data as association, i}
    ...
    >
      ...
      >
      <td class="text-sm p-4"
        >{association.substitute_response?.current
          ? association.substitute_response.current.name
          : ""}</td
      >
      {#if env.PUBLIC_ENABLE_CONFEDERATIONS}
        <td class="text-sm p-4"
          >{association.trade_union_response?.current
            ? association.trade_union_response.current.name
            : ""}</td
        >
      {/if}
      {#if env.PUBLIC_SHOW_PRIMARY_ASSOCIATION}
        <td class="text-sm p-4"
          >{association.primary_response?.current
            ? association.primary_response.current.name
            : ""}</td
        >
      {/if}
