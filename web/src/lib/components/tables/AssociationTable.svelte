  ...
  onMount(async () => {
    ...
    for (const outer of res.associations.objects) {
      ...
      for (const a of filtered as unknown as EnrichedRow[]) {
        ...
        a.person_response = resolveCurrent(a.person_response, anchor)
        a.association_type_response = resolveCurrent(
          a.association_type_response,
          anchor
        )
        a.trade_union_response = resolveCurrent(a.trade_union_response, anchor)
        a.substitute_response = resolveCurrent(a.substitute_response, anchor)
