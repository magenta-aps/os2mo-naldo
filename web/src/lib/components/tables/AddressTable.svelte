<script lang="ts">
  ...
  type EnrichedRow = EnrichResponses<
    Row,
    "address_type_response" | "visibility_response"
  >
  ...
  onMount(async () => {
    ...
    for (const outer of res.addresses.objects) {
      ...
      for (const a of filtered as unknown as EnrichedRow[]) {
        ...
        a.address_type_response = resolveCurrent(a.address_type_response, anchor)!
        a.visibility_response = resolveCurrent(a.visibility_response, anchor)
      }
