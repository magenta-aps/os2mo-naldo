<script lang="ts">
  ...
  // Catches items approved by their step but since edited into invalidity;
  // they land in the warning list below instead of being submitted.
  ...
  // See mappers.ts for the retry semantics.
  const uuids = getUserflowUuids(get(ituserInfo).map((item) => item._key))
  ...
  $: ituserSections = $ituserInfo
    ...
    .map(({ ituser, index }) => ({
      ...
      rows: [
        ...
        // One bulleted row, like manager responsibilities.
        {
