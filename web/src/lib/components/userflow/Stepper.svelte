<script lang="ts">
  ...
  // One combined store of every step's validity, indexable in the markup
  // ($-prefix auto-subscription only works on top-level identifiers).
...
>
  {#each steps as stepDef, index}
    ...
    <button
      ...
      on:click={() => {
        // The first step anchors the flow: every other step is reachable only
        // once it validates (its data is what the rest of the wizard creates
        // against).
