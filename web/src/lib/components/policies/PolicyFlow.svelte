<script lang="ts">
  ...
  // Any rule may carry an optional filter: a CEL expression returning one or
  // more access-check specs {collection, filter, check, field}, interpreted by
  // the backend. It is not collection-specific, so a single example placeholder
  // fits every rule. Reactive so it follows the active locale.
  $: filterPlaceholder = `${$_("entity_filter_hint_suffix")}, e.g. {"collection": "ituser", "check": "IN", "field": input.uuid, "filter": {"employee": {"uuids": [token.uuid]}}}`
  ...
  const submit = async () => {
    ...
    const declaredRules = rules
      ...
      .map((rule) => ({
        ...
        // The filter (a CEL expression) passes through verbatim; an empty one
        // means no entity restriction. Any rule may carry one.
        ...
        {#each rules as rule, i}
          <div class="bg-base-200 rounded-sm">
            <div class="p-6 space-y-4">
              ...
              <!-- Optional filter: a CEL expression returning one or more
                   access-check specs restricting the grant to matching objects.
                   Interpreted by the backend, so it is offered for every rule. -->
              <div class="form-control pb-1">
                ...
                <textarea
                  ...
                  rows="2"
                  class="textarea textarea-bordered textarea-sm rounded font-mono text-sm w-full focus:outline-0 focus:textarea-primary"
                />
