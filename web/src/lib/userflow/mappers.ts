...
// Pure and uuid-injected, so the whole payload is unit-testable.

// Minted once per wizard run: children reference the employee/ituser they
// belong to within the same mutation, and a retry after a partial failure
// re-sends the same uuids, erroring on what exists instead of creating a
// duplicate person. resetUserflowStores() clears them.
...
// Keyed by _key, not index: removing an ituser between summary visits must not
// re-pair a minted uuid with a different item on retry.
...
// Empty = indistinguishable from pristine, so not worth warning about. Dates
// are ignored (seeded), as is the Skattestyrelsen-seeded account name.
