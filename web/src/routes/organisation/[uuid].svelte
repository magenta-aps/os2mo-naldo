<script lang="ts">
  ...
  `
	const fetchOrgGraph = async (query: string) => {
		...
		if (response.data) {
			return response.data.org_units[0].objects[0]
		} else {
	...
	// Using service API
	// const fetchOrgDetails = async (uuid: string) => {
	// 	const url = `http://localhost:5000/service/ou/${uuid}/details/org_unit`
				...
				<div>
					...
					<div class="tabs tabs-boxed bg-base-300 flex justify-center">
						{#each menuItems as menuItem, index}
							...
							{:else}
								<a href="" on:click={() => (currentTab = index)} class="tab tab-boxed">{menuItem}</a
								>
							{/if}
						{/each}
					</div>
